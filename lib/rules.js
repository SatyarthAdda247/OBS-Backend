'use strict';
/**
 * Stateless ticker-rule engine.
 *
 * Deliberately pure: no filesystem writes, no in-memory session state. Vercel
 * functions are stateless and their filesystem is read-only, so the rule table is
 * a committed JSON file. Editing rules is a commit + redeploy, not a runtime write.
 */
...
const MIN_TITLE_MATCH_RATIO = 0.6;
...
function significantTokens(channelName) {
...
function isBetterTitleMatch(a, b) {
...
function resolveRule({ streamKey, channelName, title }) {
